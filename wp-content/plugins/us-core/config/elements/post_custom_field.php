...
if ( function_exists( 'acf_get_field_groups' ) AND $acf_groups = acf_get_field_groups() ) {
	foreach ( $acf_groups as $group ) {
		...
		foreach ( acf_get_fields( $group['ID'] ) as $field ) {
			...
			// Exclude specific ACF types, which can't output strings
			if ( ! in_array( $field['type'], $non_string_types ) ) {
				$acf_custom_fields[ $field_name_prefix . $field['name'] ] = $group['title'] . ': ' . $field['label'];
			}

			// Add image types for show_if conditions
			if ( $field['type'] == 'image' ) {
				$image_fields[] = $field_name_prefix . $field['name'];
			}
		}
	}
}
