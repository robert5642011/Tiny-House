...
if ( ! function_exists( 'us_get_taxonomies' ) ) {
	...
	function us_get_taxonomies( $public_only = FALSE, $show_slug = TRUE, $output = '', $key_prefix = '' ) {
		...
		foreach ( $taxonomies as $taxonomy ) {
			...
			// Exclude not used Link Category
...
if ( ! function_exists( 'us_get_elm_link_options' ) ) {
	...
	function us_get_elm_link_options() {
		static $link_options = array();
		if ( ! empty( $link_options ) ) {
			return (array) $link_options;
		}
			...
			foreach ( $ids as $id ) {
				...
				*/
				...
				foreach ( array( '_wpb_post_custom_css', usb_get_key_custom_css() ) as $meta_key ) {
					if ( ! empty ( $meta_key ) AND $post_custom_css = get_metadata( 'post', $id, $meta_key, TRUE ) ) {
...
if ( ! function_exists( 'us_output_design_css' ) ) {
	 ...
	 * TODO: After implementation #2547 get rid of all requests!
	 *
	 * @return string
	 */
	function us_output_design_css( $custom_posts = [] ) {
		...
		// List of already parsed page blocks to prevent excessive load to server
			...
			foreach ( $map_values as $mask_name => &$mask_props ) {
				if ( count( $mask_props ) === count( $css_mask[ $mask_name ] ) OR $mask_name == 'background' ) {
					...
					if ( strpos( $mask_name, '-shadow' ) !== FALSE ) {
						...
						if ( array_sum( array_map( 'intval', $_value ) ) === 0 ) {
							continue;
						}
					}
			...
			$func_replace_callback = function ( $matches ) use ( $elm_context, $grid_object_type ) {
				...
				if ( $matches[0] === '{{comment_count}}' AND $object_type === 'post' ) {
					...
					if ( function_exists( 'get_field' ) ) {
						...
						// If the value has the "option|" prefix, use the "option" instead of object ID
						if ( strpos( $matches[1], 'option|' ) === 0 ) {
							$meta_value = get_field( substr( $matches[1], 7 ), 'option' );
						} else {
...
if ( ! function_exists( 'us_get_edit_post_link' ) ) {
	...
	function us_get_edit_post_link( $post_id, $post_type = 'page' ) {
		...
		if ( us_get_option( 'live_builder' ) AND class_exists( 'USBuilder' ) ) {
			$link = USBuilder::get_edit_permalink( $post_id );
...
if ( ! function_exists( 'us_conditions_are_met' ) ) {
	...
	function us_conditions_are_met( $conditions, $conditions_operator ) {
		...
		// Get current page ID or term object
		$current_post_id = $current_term = NULL;
		if ( is_tax() OR is_tag() OR is_category() OR is_author() ) {
			$current_term = get_queried_object();

		} elseif ( is_404() AND ( $page_404_ID = us_get_option( 'page_404' ) ) !== 'default' ) {
			$current_post_id = $page_404_ID;

		} elseif ( is_search() AND ( $search_page_ID = us_get_option( 'search_page' ) ) !== 'default' ) {
			$current_post_id = $search_page_ID;
		 ...
		 * @param string $separator The separator for a list of values
		 * @return bool True if successful, otherwise False
		 */
		$func_compare_values = function( $needle, $haystack, $mode = '=', $separator = ',' ) {

			if ( is_string( $haystack ) AND strpos( $haystack, $separator ) !== FALSE ) {
				$haystack = explode( $separator, $haystack );
			}
		...
		foreach( $conditions as $i => $condition ) {
			...
			if ( $condition_param === 'post_type' ) {
				$condition_result = $func_compare_values( get_post_type(), us_arr_path( $condition, 'post_type' ), $mode );

			} elseif ( $condition_param === 'post_id' AND ! empty( $current_post_id ) ) {
				$condition_result = $func_compare_values( $current_post_id, us_arr_path( $condition, 'post_value' ), $mode );

			} elseif ( $condition_param === 'user_role' ) {
				foreach( (array) wp_get_current_user()->roles as $user_role ) {
					$condition_result = ( $condition_result OR $func_compare_values( $user_role, us_arr_path( $condition, 'user_role' ), $mode ) );
				}

			} elseif ( $condition_param === 'user_state' ) {
				$user_state = is_user_logged_in() ? 'logged_in' : 'logged_out';
				$condition_result = $func_compare_values( $user_state, us_arr_path( $condition, 'user_state' ) );
				...
				if ( empty( $current_term ) ) {
					...
					if ( ! $terms = get_the_terms( $current_post_id, $condition_taxonomy ) ) {
						$terms = array( 1 => '' );
					}
				...
				foreach( $terms as $term ) {
					...
					$condition_result = $func_compare_values( $term_value, us_strtolower( $condition_value ), $mode );
			...
			} elseif ( $condition_param === 'custom_field' AND $meta_key = us_arr_path( $condition, 'cf_name' ) ) {
				if ( ! empty( $current_term ) ) {
					$object_id = $current_term;
					$meta_type = 'term';
				} else {
				...
				$condition_result = $func_compare_values( $meta_value, us_arr_path( $condition, 'cf_value', '' ), $mode );
				...
				$condition_result = $func_compare_values( $cart_status, us_arr_path( $condition, 'cart_status' ) );
				 ...
				 */
				$time_pairs = array(
					'd' => us_arr_path( $condition, 'time_day', '' ),
					'H' => us_arr_path( $condition, 'time_hour', '' ),
					'i' => us_arr_path( $condition, 'time_minute', '' ),
					'm' => us_arr_path( $condition, 'time_month', '' ),
					's' => '00', // sec.
					'Y' => us_arr_path( $condition, 'time_year', '' ),
				);
				$_timestamp = wp_date( 'U', strtotime( strtr( 'Y-m-d H:i:s', $time_pairs ) ) );

				// Definition of mode
				$mode = ( us_arr_path( $condition, 'time_operator', /* default */'since' ) === 'since' )
					? '>='
					: '<=';

				$condition_result = $func_compare_values( wp_date( 'U' ), $_timestamp, $mode );
