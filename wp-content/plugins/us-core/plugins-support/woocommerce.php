...
if ( ! defined( 'US_DEV' ) AND ! us_get_option( 'optimize_assets', 0 ) ) {
...
if ( ! function_exists( 'us_checkout_page_content' ) ) {
	...
	function us_checkout_page_content( $content ) {
		...
		// Checking if we should display a checkout endpoint instead of our elements
		if (
			function_exists( 'is_wc_endpoint_url' )
			AND is_wc_endpoint_url()
			AND (
				...
				// Case when `us_checkout_*` elements are in Content Template
				OR (
			...
			$content_shortcodes = "[vc_row][vc_column][woocommerce_checkout][/vc_column][/vc_row]";
			$content = do_shortcode( $content_shortcodes );

			// If checkout is disabled for unregistered - display a message isntead of our elements
		} elseif (
			...
			// If the content contains checkout elements, then wrap it in the form
