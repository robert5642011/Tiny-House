...
if ( ! empty( $custom_icon ) AND ! empty( $custom_link ) ) {
	...
	if ( $custom_link_new_tab ) {
		$custom_link_atts['target'] = '_blank';
		$custom_link_atts['rel'] = 'noopener';
	}
	$social_links_html .= '<a' . us_implode_atts( $custom_link_atts ) . '>' . us_prepare_icon_tag( $custom_icon ) . '</a>';
}
