...
if ( $us_elm_context == 'grid' ) {
	...
	} elseif ( is_tax() OR is_tag() OR is_category() OR is_author() ) {
...
$postID = apply_filters( 'us_tr_object_id', $postID );
...
if ( $key == 'custom' AND ! empty( $custom_key ) ) {
	...
	if ( function_exists( 'get_field_object' ) ) {
		...
		// If the value has the "option|" prefix, use the "option" instead of post ID
		// https://www.advancedcustomfields.com/resources/get-values-from-an-options-page/
		if ( strpos( $key, 'option|' ) === 0 ) {
			$acf_obj = get_field_object( substr( $key, 7 ), 'option' );
		} elseif ( $postID ) {
	...
	// Add <p> and <br> if the value has 'End Of Line' symbols
	if (
		is_string( $value )
		AND $type == 'text'
