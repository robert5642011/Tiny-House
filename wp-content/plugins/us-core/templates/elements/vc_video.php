...
if (
	...
	// If ACF field returns an iframe, get link from value and replace with current
	if ( ! empty( $result[1][0] ) ) {
		...
		// in case the ACF field is not iframe and not empty
	} elseif ( $value !== '' ) {
		$link = $value;

		// in other cases do not output the element
	} else {
		return;
	}
}
