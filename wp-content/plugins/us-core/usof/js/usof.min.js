...
var value=this.getValue(),isVC=this.$row.hasClass('us_select_for_vc'),$addDesc=$('.us-grid-layout-desc-add',isVC?this.$row.parent():this.$row),$editLink=$('.us-grid-layout-desc-edit',isVC?this.$row.parent():this.$row);if($.isNumeric(value)){$('.edit-link',$editLink).attr('href',(this.$container.data('edit_link')||'').replace('%d',value));$addDesc.addClass('hidden');$editLink.removeClass('hidden')}else{$addDesc.removeClass('hidden');$editLink.addClass('hidden')}},setValue:function(value,quiet){if(!$('option[value="'+value+'"]',this.$input).length){value=$('option:first',this.$input).attr('value')||''}
this.parentSetValue(value,quiet);this._changeSelect.call(this);this._toggleGridLayoutDesc.call(this)}}}(jQuery);!function($,undefined){var _window=window,_document=document;if(_window.$usof===undefined){return}
$usof.field.font={init:function(options){this.parentInit(options);this.$preview=this.$row.find('.usof-font-preview');this.$weightsContainer=this.$row.find('.usof-checkbox-list');this.$weightCheckboxes=this.$weightsContainer.find('.usof-checkbox');this.$weights=this.$weightsContainer.find('input');this.previewParams=this.$row.find('.usof-preview-params-json')[0].onclick()||{};this.notLoadedFonts=[];this.fontInited=!1;this.$fieldFontName={};var $autocomplete=$('.type_autocomplete',this.$row);this.fontsGroupKeys=$autocomplete[0].onclick()||{};$autocomplete.removeAttr('onclick');if($autocomplete.length){this.$fieldFontName=new $usof.field($autocomplete);this.$fieldFontName.trigger('beforeShow')}
...
this.$fieldFontName.on('change',function(){this.isCurFontUploaded=this.fontHasGroup.call(this,this.$fieldFontName.getValue(),'uploaded');this.setValue(this._getValue.call(this))}.bind(this)).on('data.loaded',function(){initNotLoadedFonts.call(this)}.bind(this));this.$weights.on('change',function(){this.setValue(this._getValue.call(this))}.bind(this));if(this.previewParams.color!=undefined){$usof.instance.fields[this.previewParams.color].on('change',function(){this.$preview.css('color',$usof.instance.fields[this.previewParams.color].getValue())}.bind(this))}
if(this.previewParams.fontsize!=undefined){$usof.instance.fields[this.previewParams.fontsize].on('change',function(){this.$preview.css('font-size',$usof.instance.fields[this.previewParams.fontsize].getValue())}.bind(this))}
if(this.previewParams.lineheight!=undefined){$usof.instance.fields[this.previewParams.lineheight].on('change',function(){this.$preview.css('line-height',$usof.instance.fields[this.previewParams.lineheight].getValue())}.bind(this))}
if(this.previewParams.fontweight!=undefined){$usof.instance.fields[this.previewParams.fontweight].on('change',function(){this.$preview.css('font-weight',$usof.instance.fields[this.previewParams.fontweight].getValue())}.bind(this))}
if(this.previewParams.letterspacing!=undefined){$usof.instance.fields[this.previewParams.letterspacing].on('change',function(){this.$preview.css('letter-spacing',$usof.instance.fields[this.previewParams.letterspacing].getValue())}.bind(this))}
if(this.previewParams.texttransform!=undefined){$usof.instance.fields[this.previewParams.texttransform].on('change',function(){this.$preview.css('text-transform',$usof.instance.fields[this.previewParams.texttransform].getValue())}.bind(this))}
if(this.previewParams.fontstyle!=undefined){$usof.instance.fields[this.previewParams.fontstyle].on('change',function(){if($usof.instance.fields[this.previewParams.fontstyle].getValue().indexOf("italic")!=-1){this.$preview.css('font-style','italic')}else{this.$preview.css('font-style','normal')}}.bind(this))}
...
if(this.previewParams.fontweight==undefined){if(fontWeights.length==0){this.$preview.css('font-weight','')}else{this.$preview.css('font-weight',parseInt(fontWeights[0]))}}
...
$usof.field.editor={init:function(){this.$container=$('.usof-editor',this.$row);this.$container.find('script.usof-editor-template').remove();this.originalEditorId=this.$input.data('editor-id')||'usof_editor';this.originalEditorSettings=_window.tinyMCEPreInit.mceInit[this.originalEditorId]||{};this.editorSettings=$('.usof-editor-settings',this.$row)[0].onclick()||{};this.editorId=this.originalEditorId+$usof.uniqid();this.$input.attr('id',this.editorId);this._events={changeField:this._changeField.bind(this),changeTinymceContent:this._changeTinymceContent.bind(this)};this.$container.on((this.isUSBuilder()?'input':'change'),'textarea',this._events.changeField);this.initEditor()},initEditor:function(){if(!_window.wp||!_window.wp.editor)return;_window.tinymce.on('AddEditor',function(e){if(e.editor.id!==this.editorId)return;var eventName=this.isUSBuilder()?'input NodeChange':'change NodeChange';e.editor.off(eventName).on(eventName,this._events.changeTinymceContent)}.bind(this),!0);var pid=setTimeout(function(){var editorSettings={quicktags:!0,tinymce:this.editorSettings.tinymce||{},mediaButtons:(this.editorSettings.media_buttons!==undefined)?this.editorSettings.media_buttons:!0},qtSettings={id:this.editorId,buttons:"strong,em,link,block,del,ins,img,ul,ol,li,code,more,close"},settingsFields=['content_css','toolbar1','toolbar2','toolbar3','toolbar4','theme','skin','language','formats','relative_urls','remove_script_host','convert_urls','browser_spellcheck','fix_list_elements','entities','entity_encoding','keep_styles','resize','menubar','branding','preview_styles','end_container_on_empty_block','wpeditimage_html5_captions','wp_lang_attr','wp_keep_scroll_position','wp_shortcut_labels','plugins','wpautop','indent','tabfocus_elements','textcolor_map','textcolor_rows',];settingsFields.forEach(function(setting,index){if(this.originalEditorSettings[setting]!==undefined){editorSettings.tinymce[setting]=this.originalEditorSettings[setting]}}.bind(this));editorSettings.tinymce.setup=$.noop;_window.wp.editor.initialize(this.editorId,editorSettings);_window.quicktags(qtSettings);this.switchEditors('tinymce');clearTimeout(pid)}.bind(this),1)},switchEditors:function(mode){mode=(''+mode).toLowerCase();$('#'+this.editorId+'-'+(mode==='tinymce'?'tmce':'html'),this.$container).trigger('click')},_changeField:function(e){this.trigger('change',e.currentTarget.value)},_changeTinymceContent:function(e){var mceValue=''+_window.tinymce.get(this.editorId).getContent(),currentValue=''+this.getValue();if(currentValue===mceValue){return}
this.$input.val(mceValue);this.trigger('change',mceValue)},setValue:function(value,quiet){if(!!_window.tinyMCE&&!!_window.tinyMCE.get(this.editorId)){_window.tinyMCE.get(this.editorId).setContent(value)}else{this.$input.val(value)}
if(quiet){this.trigger('change',value)}},getValue:function(){return this.$input.val()||''}}}(jQuery);!function($,undefined){var _window=window,_document=document;if(_window.$usof===undefined){return}
...
this.$body.off('mousemove',this._events._maybeDragMove);this._detach();this.$body.on('mousemove',this._events._dragMove)},_detach:function(event){var offset=this.$draggedElm.offset();this.elmPointerOffset[0]-=offset.left;this.elmPointerOffset[1]-=offset.top;this.$dragshadow.css({width:this.$draggedElm.outerWidth(),height:this.$draggedElm.outerHeight()}).insertBefore(this.$draggedElm);this.$draggedElm.css({position:'absolute','pointer-events':'none',zIndex:10000,width:this.$draggedElm.width(),height:this.$draggedElm.height()}).css(offset).appendTo(this.$body);this.detached=!0},_dragMove:function(event){event.stopPropagation();this.$draggedElm.css({left:event.pageX-this.elmPointerOffset[0],top:event.pageY-this.elmPointerOffset[1]});if(this._isInBlindSpot(event)){return}
var elm=event.target;for(var level=0;level<=2;level ++,elm=elm.parentNode){if(this._isShadow(elm)){return}
...
nextElm=nextElm.previousSibling}
this.$dragshadow[shadowAtLeft?'insertAfter':'insertBefore'](elm);this._dragDrop(event);break}}},_dragDrop:function(event){this._updateBlindSpot(event)},_dragEnd:function(event){this.$body.off('mousemove',this._events._maybeDragMove).off('mousemove',this._events._dragMove);this.$window.off('mouseup',this._events._dragEnd);if(this.detached){this.$draggedElm.removeAttr('style').insertBefore(this.$dragshadow);this.$dragshadow.detach();var value=[],items=$('.usof-upload-preview-file',this.$previewContainer).toArray()||[];for(var k in items){if(items[k].hasAttribute('data-id')){value.push(items[k].getAttribute('data-id'))}}
...
if(this._getColorValue('color_shadow')!=''){style.default+='box-shadow:'+this.groupParams.getValue('shadow_offset_h')+' '+this.groupParams.getValue('shadow_offset_v')+' '+this.groupParams.getValue('shadow_blur')+' '+this.groupParams.getValue('shadow_spread')+' '+this._getColorValue('color_shadow')+' ';if($.inArray('1',this.groupParams.getValue('shadow_inset'))!==-1){style.default+='inset'}
style.default+='!important;'}
this.$container.usMod('hov',this.groupParams.getValue('hover'));if(this.groupParams.getValue('hover')=='fade'){style.hover+='background:'+colorBgHover+' !important;'}else if(colorBgHover=='transparent'){style.hover+='background:'+colorBgHover+' !important;'}
if(this._getColorValue('color_shadow_hover')!=''){style.hover+='box-shadow:'+this.groupParams.getValue('shadow_hover_offset_h')+' '+this.groupParams.getValue('shadow_hover_offset_v')+' '+this.groupParams.getValue('shadow_hover_blur')+' '+this.groupParams.getValue('shadow_hover_spread')+' '+this._getColorValue('color_shadow_hover')+' ';if($.inArray('1',this.groupParams.getValue('shadow_hover_inset'))!==-1){style.hover+='inset'}
