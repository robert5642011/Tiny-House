...
<?php if ( ! empty( $this->authors ) ) : ?>
	...
	<p><?php echo 'To make it simpler for you to edit and save the imported content, you may want to reassign the author of the imported item to an existing user of this site, such as your primary administrator account.'; ?></p>
...
<?php endif; ?>
	...
	<p class="submit"><input type="submit" class="button" value="<?php echo esc_attr( 'Submit' ); ?>" /></p>
</form>
<?php
	}
			...
			if ( ! empty( $post['comments'] ) ) {
				...
				foreach ( $newcomments as $key => $comment ) {
					...
					if ( ! $post_exists || ! comment_exists( $comment['comment_author'], $comment['comment_date'] ) ) {
						...
						if ( isset( $comment_data['commentmeta'] ) ) {
	...
	function process_menu_item( $item ) {
		...
		// UpSolution edit: Support for Exclude Rows Menu feature
		if ( isset( $_menu_item_remove_rows ) ) {
			update_post_meta( $id, '_menu_item_remove_rows', intval( $_menu_item_remove_rows ) );
		}
		// UpSolution edit: Support for Menu items to be displayed as button
		if ( isset( $_menu_item_btn_style ) ) {
			update_post_meta( $id, '_menu_item_btn_style', intval( $_menu_item_btn_style ) );
		}
